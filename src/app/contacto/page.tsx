        ...
        <div className="max-w-container-max mx-auto grid grid-cols-1 lg:grid-cols-2 gap-gutter items-center">
          <div className="space-y-6">
            <h1 className="font-display-lg text-headline-lg-mobile md:text-display-lg text-primary">
              Agenda tu reparación con expertos.
            </h1>
            <p className="font-body-lg text-body-lg text-on-surface-variant max-w-lg">
              Estamos en Coquimbo listos para devolverle la vida a tu vehículo. Cotiza en línea, llámanos directamente o visítanos en nuestro taller mecánico especializado en cristales, desabolladura y pintura.
            </p>
            {/* Direct Action Buttons */}
            <div className="flex flex-col sm:flex-row gap-4 pt-4">
              <a className="flex justify-center items-center gap-2 font-label-bold text-label-bold bg-secondary text-on-secondary px-6 py-3 rounded-lg hover:opacity-90 transition-opacity shadow-md w-full sm:w-auto" href="tel:+56912345678">
                <span className="material-symbols-outlined fill">call</span>
                Llamar Directo
              </a>
              <a className="flex justify-center items-center gap-2 font-label-bold text-label-bold bg-[#25D366] text-white px-6 py-3 rounded-lg hover:opacity-90 transition-opacity shadow-md w-full sm:w-auto" href="https://wa.me/56912345678" target="_blank" rel="noreferrer">
            ...
            <form className="space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <input className="w-full bg-surface-bright border-outline-variant text-on-surface focus:border-primary focus:ring-primary rounded-lg font-body-md text-body-md p-3 transition-colors border" id="name" name="name" placeholder="Ej: Juan Pérez" type="text" />
                </div>
                {/* Phone */}
                <div className="space-y-2">
                  <label className="font-label-bold text-label-bold text-primary block" htmlFor="phone">Teléfono / WhatsApp</label>
                  <input className="w-full bg-surface-bright border-outline-variant text-on-surface focus:border-primary focus:ring-primary rounded-lg font-body-md text-body-md p-3 transition-colors border" id="phone" name="phone" placeholder="+56 9 " type="tel" />
                </div>
              </div>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                {/* Vehicle Make/Model */}
                <div className="space-y-2">
                  ...
                  <input className="w-full bg-surface-bright border-outline-variant text-on-surface focus:border-primary focus:ring-primary rounded-lg font-body-md text-body-md p-3 transition-colors border" id="vehicle" name="vehicle" placeholder="Ej: Toyota Hilux 2020" type="text" />
                </div>
                {/* Service Type */}
                <div className="space-y-2">
                  <label className="font-label-bold text-label-bold text-primary block" htmlFor="service">Tipo de Servicio Principal</label>
                  <select className="w-full bg-surface-bright border-outline-variant text-on-surface focus:border-primary focus:ring-primary rounded-lg font-body-md text-body-md p-3 transition-colors border" id="service" name="service" defaultValue="">
                    <option disabled value="">Selecciona un servicio</option>
                    <option value="cristales">Cambio de Cristales</option>
            ...
            <div className="bg-surface-container-low rounded-xl border border-outline-variant overflow-hidden shadow-sm flex-grow flex flex-col">
              <div className="p-6 border-b border-outline-variant bg-surface-container-lowest">
                ...
                <p className="font-body-md text-body-md text-on-surface-variant flex items-start gap-2 mt-3">
                  Av. Principal 1234, Barrio Industrial,<br/> Coquimbo, Región de Coquimbo.
                </p>
              </div>
              {/* Map Placeholder */}
              <div className="flex-grow min-h-[250px] relative bg-surface-variant">
                <img alt="Mapa de ubicación Coquimbo" className="absolute inset-0 w-full h-full object-cover opacity-80" src="https://lh3.googleusercontent.com/aida-public/AB6AXuB89Er67GHKIlHsbhKxjhL3YUBcOTVpto2cdu8pAKnhqzFLdutzHdlMjvGcw6sOlkQqESTemhWDSzMpXv1kX5BbN_vrKnb7fNkc2rf5dfTiFZgiRoL9z6ixi8FnndalrZ46CcTwC3H1ZOit9tnfS3f1MS7tYoXU7chKPzGy4FkDwEM2EWenXny_FUGGeSQJ2xhti4Hl9ZJdSTa5D4eJeah5HnZ60LYUHzufxlAL5fIQSp7Dn5eUDSqYrg6o8-Lg_1ok8yKr0TAq6nU" />
                {/* Overlay pin */}
                <div className="absolute inset-0 flex items-center justify-center pointer-events-none">
                  <div className="bg-secondary text-white p-2 rounded-full shadow-lg transform -translate-y-4 flex items-center justify-center">
                    <span className="material-symbols-outlined fill">pin_drop</span>
                  </div>
                </div>
              </div>
              <a className="block w-full text-center bg-surface-container-highest py-3 font-label-bold text-label-bold text-primary hover:bg-surface-dim transition-colors" href="https://maps.google.com" target="_blank" rel="noreferrer">
